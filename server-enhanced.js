...
app.get('/exports/fra/backups/hard/:filename', requireAdmin, (req, res) => {
  try {
    ...
    if (!filename || !/^fra_tracker_backup_.*\.json$/i.test(filename)) {
      return sendError(res, 404, 'NOT_FOUND', 'Hard backup file not found');
    }

    const filePath = path.join(FRA_HARD_BACKUP_DIR, filename);
    if (!fs.existsSync(filePath)) {
      return sendError(res, 404, 'NOT_FOUND', 'Hard backup file not found');
    }
    ...
    return sendError(res, 500, 'SERVER_ERROR', 'Failed to open FRA hard backup file');
  }
});
